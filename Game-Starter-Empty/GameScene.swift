...
class GameScene: SKScene {
    ...
    func createSquares(name: String) {
        ...
        let square = SKSpriteNode(texture: nil, color: .red, size: CGSize(width: 80, height: 80))
        square.name = name
        ...
        square.position = CGPoint(x: randomNumber(), y: 40)
        ...
        let action = SKAction.customAction(withDuration: 2.0, actionBlock: { (square, _) in
            ...
            if square.position.y >= self.frame.size.height{
